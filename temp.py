...
#     # def batch_cosine_similarity(self, x, batch_size, mask_nodes):
#     #     # 提取 mask_nodes 中的特征
#     #     x_masked = x[mask_nodes]  # 只取出 mask_nodes 对应的特征
#     #     N = x.shape[0]  # 总节点数（所有节点）
#     #     M = x_masked.shape[0]  # mask_nodes 中的节点数
#     #
#     #     # 不再初始化整个矩阵，改为按需保存分块结果
#     #     similarity_matrix_parts = []
#     #
#     #     # 分块计算余弦相似度
#     #     for i in range(0, M, batch_size):
#     #         end_i = min(i + batch_size, M)
#     #         similarity_block = []  # 用于保存该行的块
#     #
#     #         for j in range(0, N, batch_size):
#     #             end_j = min(j + batch_size, N)
#     #
#     #             # 对小块进行余弦相似度计算，并将结果放入CPU中
#     #             block_similarity = F.cosine_similarity(
#     #                 x_masked[i:end_i].unsqueeze(1),
#     #                 x[j:end_j].unsqueeze(0),
#     #                 dim=-1
#     #             ).cpu()  # 将计算结果移到CPU以节省GPU显存
#     #
#     #             similarity_block.append(block_similarity)
#     #
#     #         # 拼接这一行的相似度块
#     #         similarity_block = torch.cat(similarity_block, dim=1)
#     #         similarity_matrix_parts.append(similarity_block)
#     #
#     #     # 将所有分块结果合并回完整的相似度矩阵
#     #     similarity_matrix = torch.cat(similarity_matrix_parts, dim=0)
#     #
#     #     return similarity_matrix
#
#     # def batch_cosine_similarity(self, x, batch_size, mask_nodes):
...
#     #     edge_nodeID1 = []
#     #     edge_nodeID2 = []
#     #     # 初始化相似度矩阵，只计算 mask_nodes 和所有节点的相似度
#     #     similarity_matrix = torch.zeros((M, N)).to(x.device)
#     #     with torch.no_grad():
#     #     # 分块计算余弦相似度
#     #         for i in range(0, M, batch_size):
#     #             end_i = min(i + batch_size, M)
...
#
#         with torch.no_grad():
#             # 外层循环：每次处理 mask_nodes 中的 batch_size 个节点
#             for i in range(0, M, batch_size):
#                 end_i = min(i + batch_size, M)
#                 x_masked_batch = x_masked[i:end_i]  # 当前 batch_size 个节点的特征
#                 batch_node_ids = mask_nodes[i:end_i]  # 当前 batch_size 个节点的索引
#
#                 # 初始化相似度矩阵为 (当前批次大小, 总节点数)
#                 similarity_matrix = torch.zeros((end_i - i, N), device=x.device)
#
#                 # 内层循环：将当前 mask_nodes 的批次与所有节点的特征计算相似度
#                 for j in range(0, N, batch_size):
#                     end_j = min(j + batch_size, N)
#                     similarity_matrix[:, j:end_j] = F.cosine_similarity(
#                         x_masked_batch.unsqueeze(1),
#                         x[j:end_j].unsqueeze(0),
#                         dim=-1
#                     )
#
#                 # 对当前批次的每个节点，找到与之相似度最高的 k 个节点（排除自身）
#                 for idx, node_id in enumerate(batch_node_ids):
#                     # 当前节点的相似度向量 (1, N)
#                     node_similarity = similarity_matrix[idx]
#
#                     # 获取 top_k + 1 个相似节点（包括自身）
#                     top_k_values, top_k_indices = torch.topk(node_similarity, k=k+1)
#
#                     # 排除自身并仅保留 top_k 个节点
#                     top_k_indices = top_k_indices[top_k_indices != node_id][:k]
#
#                     # 添加边
#                     edge_nodeID1.extend([node_id.item()] * len(top_k_indices))
#                     edge_nodeID2.extend(top_k_indices.tolist())
#         print('--------------------------edge_nodeID1----------------------')
#         print(edge_nodeID1)
#         print('--------------------------edge_nodeID2----------------------')
#         print(edge_nodeID2)
...
#
#         edge_nodeID1, edge_nodeID2 = self.batch_top_k_cosine_similarity(enc_rep_nomask_reduced, batch_size=2048, mask_nodes = mask_nodes, k=1)
#
...
class PreModel(nn.Module):
    ...
    def mask_attr_prediction(self, g, x):
        ...
        similarity_matrix = self.batch_cosine_similarity(enc_rep_nomask_reduced, batch_size=2048)

...
def pretrain(model, graph, feat, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob, logger=None):
    ...
    for epoch in epoch_iter:
        ...
        if (epoch + 1) % 100 == 0:
            acc, _ = node_classification_evaluation(model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, linear_prob, mute=True)
            acc_list.append(acc)

    print(f"# max_acc: {max(acc_list):.4f}")
...
def main(args):
    ...
    #日志类
    logyyx = Logger(f'log/{dataset_name}_logging_{time_str}.log', logging.DEBUG, logging.DEBUG)
    logyyx.info(f"增加注意力机制、投影、增加边k = 3")
    ...
    logyyx.info(f"acc_list:  {acc_list}")

    logyyx.info(f"# final_acc: {final_acc:.4f}±{final_acc_std:.4f}")
    logyyx.info(f"# early-stopping_acc: {estp_acc:.4f}±{estp_acc_std:.4f}")
